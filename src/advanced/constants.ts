/**
 * 쇼핑카트 애플리케이션의 비즈니스 상수 정의 (TypeScript)
...
// 상품 기본 정보
export const PRODUCT_INFO: readonly Product[] = [
  {
...
// 개별 상품 할인율 매핑
export const INDIVIDUAL_DISCOUNT_RATES: Record<ProductId, number> = {
  [PRODUCT_IDS.KEYBOARD]: DISCOUNT_RATES.KEYBOARD_BULK,
  [PRODUCT_IDS.MOUSE]: DISCOUNT_RATES.MOUSE_BULK,
  [PRODUCT_IDS.MONITOR_ARM]: DISCOUNT_RATES.MONITOR_ARM_BULK,
  [PRODUCT_IDS.LAPTOP_POUCH]: DISCOUNT_RATES.LAPTOP_POUCH_BULK,
  [PRODUCT_IDS.SPEAKER]: DISCOUNT_RATES.SPEAKER_BULK,
} as const;
...
export const QUANTITY_THRESHOLDS = {
  ...
  LOW_STOCK_WARNING: 5, // 재고 부족 경고 임계값 (5개 이하일 때)
...
// 상품 ID 검증을 위한 Set
export const VALID_PRODUCT_IDS = new Set(Object.values(PRODUCT_IDS));

// 상품 정보 맵 (빠른 조회용)
export const PRODUCT_MAP = new Map(PRODUCT_INFO.map((product) => [product.id, product]));

// 상품 이름 맵 (빠른 조회용)
export const PRODUCT_NAME_MAP: Record<ProductId, string> = Object.fromEntries(
  PRODUCT_INFO.map((product) => [product.id, product.name]),
